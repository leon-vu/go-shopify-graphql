...
type BulkOperationService interface {
	...
	GetCurrentBulkQuery() (*models.BulkOperation, error)
	GetCurrentBulkQueryResultURL() (*string, error)
	WaitForCurrentBulkQuery(interval time.Duration) (*models.BulkOperation, error)
...
type mutationBulkOperationRunQuery struct {
	BulkOperationRunQueryResult models.BulkOperationRunQueryPayload `graphql:"bulkOperationRunQuery(query: $query)" json:"bulkOperationRunQuery"`
}

type mutationBulkOperationRunQueryCancel struct {
	BulkOperationCancelResult models.BulkOperationCancelPayload `graphql:"bulkOperationCancel(id: $id)" json:"bulkOperationCancel"`
}
...
func (s *BulkOperationServiceOp) GetCurrentBulkQuery() (*models.BulkOperation, error) {
	var q struct {
...
func (s *BulkOperationServiceOp) WaitForCurrentBulkQuery(interval time.Duration) (*models.BulkOperation, error) {
	q, err := s.GetCurrentBulkQuery()
	if err != nil {
		return q, fmt.Errorf("CurrentBulkOperation query error: %w", err)
	}

	for q.Status == models.BulkOperationStatusCreated || q.Status == models.BulkOperationStatusRunning || q.Status == models.BulkOperationStatusCanceling {
...
func (s *BulkOperationServiceOp) CancelRunningBulkQuery() error {
	...
	if q.Status == models.BulkOperationStatusCreated || q.Status == models.BulkOperationStatusRunning {
